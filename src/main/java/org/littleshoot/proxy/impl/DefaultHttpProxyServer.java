...
public class DefaultHttpProxyServer implements HttpProxyServer {
    ...
    private static final String TAG = "PROXY-" + DefaultHttpProxyServer.class.getSimpleName();
    ...
    */
    ...
    private HttpProxyServer start() {
        Log.i(TAG, "Starting proxy at address: " + this.requestedAddress);
    ...
    private void doStart() {
        ...
        case TCP:
            ...
            break;
        case UDT:
            Log.i(TAG, "Proxy listening with UDT transport");
        ...
        Log.i(TAG, "Proxy started at address: " + this.boundAddress);
    }
    ...
    private static class ServerGroup {
        ...
        synchronized private void stop() {
            Log.i(TAG, "Shutting down proxy");
            if (stopped) {
                Log.i(TAG, "Already stopped");
                return;
            }

            Log.i(TAG, "Closing all channels...");
            ...
            if (!future.isSuccess()) {
                ...
                while (iter.hasNext()) {
                    ...
                    if (!cf.isSuccess()) {
                        Log.i(TAG, "Unable to close channel.  Cause of failure for " + cf.channel() + " is " + cf.cause().getMessage());
                    }
                }
            }

            Log.i(TAG, "Shutting down event loops");
            ...
            for (EventLoopGroup group : allEventLoopGroups) {
                try {
                    ...
                    Log.w(TAG, "Interrupted while shutting down event loop");
                }
            }
            ...
            Log.i(TAG, "Done shutting down proxy");
        }
            ...
            if (requestedAddress != null) {
                ...
                if (allowLocalOnly) {
                    ...
                    try {
                        ...
                        Log.e(TAG, "Could not get local host? " + e.getMessage());
                        return new InetSocketAddress(port);
                    }
                }
            }
